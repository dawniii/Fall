...
def isFall(y_pred_label):
    windows=20
    step=20
    Labels=[]
    Labels_Num=[]
    fall_start=[]#只记录每次跌倒中的第一个跌倒窗口
    fall_end=[]
    windows_count=0
    for i in range(0,len(y_pred_label)-windows,step):#i是窗口次数
        label = stats.mode(y_pred_label[i:i + windows])[0][0]  # 出现最多的类别
        label_number = stats.mode(y_pred_label[i:i + windows])[1][0]  # 出现最多的类别的个数
        Labels.append(label)
        Labels_Num.append(label_number)
        if((windows_count==0 and label==1) or (windows_count!=0 and label==1 and Labels[ windows_count-1] != 1)):#如果该窗口为跌倒，上一个窗口不为跌倒
            fall_start.append(i)
            fall_end.append(i+windows)
        windows_count +=1#窗口数加1
    return Labels,Labels_Num,fall_start,fall_end
...
for train_index, test_index in floder.split(reshaped_segments):
    ...
    if(k_fold_num==1):
        checkpoint_save_path1 ="E:/Fall/Cogent/checkpoint/TCN/TCN_cogent1.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_TCN.load_weights(checkpoint_save_path1)
    if(k_fold_num==2):
        checkpoint_save_path1 ="E:/Fall/Cogent/checkpoint/TCN/TCN_cogent2.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_TCN.load_weights(checkpoint_save_path1)
    if(k_fold_num==3):
        checkpoint_save_path1 = "E:/Fall/Cogent/checkpoint/TCN/TCN_cogent3.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_TCN.load_weights(checkpoint_save_path1)
    ...
    if(k_fold_num==1):
        plt.savefig('E:/Fall/Cogent/figure/k1-TCN-C.png',bbox_inches='tight')
    if(k_fold_num==2):
        plt.savefig('E:/Fall/Cogent/figure/k2-TCN-C.png',bbox_inches='tight')
    if(k_fold_num==3):
        plt.savefig('E:/Fall/Cogent/figure/k3-TCN-C.png',bbox_inches='tight')
    plt.close()
    ...
    if (k_fold_num == 1):
        ...
        plt.savefig('E:/Fall/Cogent/figure/k1-Warning.svg')
    if (k_fold_num == 2):
        #plt.figure(2, figsize=(20, 10))
        plt.savefig('E:/Fall/Cogent/figure/k2-Warning.svg')
    if (k_fold_num == 3):
       # plt.figure(3,figsize=(20,10))
        plt.savefig('E:/Fall/Cogent/figure/k3-Warning.svg')
    plt.close()
