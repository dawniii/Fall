...
def m_plot(y_test_label, y_pred_label, test_index, segments, knum):
    ...
    for k in range(len(loupan_index)):
        ...
        plt.legend()
        plt.savefig('E:/Fall/Cogent/BiLSTM/figure2/loupan' + str(knum) + str(k + 1) + '.png', bbox_inches='tight')
        plt.close()
...
for train_index, test_index in floder.split(reshaped_segments):
    ...
    model_LSTM = tf.keras.models.Sequential([
        tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=64,input_shape=(X_train.shape[1],X_train.shape[2]))),
    ...
    if(k_fold_num==1):
        checkpoint_save_path1 ="E:/Fall/Cogent/BiLSTM/checkpoint/cogent1.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_LSTM.load_weights(checkpoint_save_path1)
    if(k_fold_num==2):
        checkpoint_save_path1 ="E:/Fall/Cogent/BiLSTM/checkpoint/cogent2.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_LSTM.load_weights(checkpoint_save_path1)
    if(k_fold_num==3):
        checkpoint_save_path1 = "E:/Fall/Cogent/BiLSTM/checkpoint/cogent3.ckpt"
        if os.path.exists(checkpoint_save_path1 + '.index'):
            print('-------------load the model-----------------')
            model_LSTM.load_weights(checkpoint_save_path1)
    ...
    if(k_fold_num==1):
        plt.savefig('E:/Fall/Cogent/BiLSTM/figure/k1-C.png',bbox_inches='tight')
        plt.close()
        m_plot(y_test_label, y_pred_label, test_index, new_segments_all, 1)

    if(k_fold_num==2):
        plt.savefig('E:/Fall/Cogent/BiLSTM/figure/k2-C.png',bbox_inches='tight')
        plt.close()
        m_plot(y_test_label, y_pred_label, test_index, new_segments_all, 2)
    if(k_fold_num==3):
        plt.savefig('E:/Fall/Cogent/BiLSTM/figure/k3-C.png',bbox_inches='tight')
        plt.close()
